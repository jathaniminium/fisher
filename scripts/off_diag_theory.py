...
for l in range(sim_runs):
    ...
    for i in range(nsims):
        ...
        mapT = np.random.normal(loc=0.0, scale=signal_scale*noise_scale, size=len(start_mapT))
        mapE = np.random.normal(loc=0.0, scale=signal_scale*noise_scale*np.sqrt(2.), size=len(start_mapE))

        #FFT and multiply by correct Cl spectra.
        T_fft = 2.*np.fft.fft(mapT)
        E_fft = 2.*np.fft.fft(mapE)
